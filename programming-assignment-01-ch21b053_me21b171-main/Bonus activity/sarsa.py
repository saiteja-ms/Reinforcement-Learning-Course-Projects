...
from policies import choose_action_epsilon

def sarsa(env, Q, episodes, gamma=0.99, alpha=0.1, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.05, print_freq=100, choose_action=choose_action_epsilon):
    """
    SARSA implementation with epsilon-greedy exploration.
    """
    ...
    for ep in tqdm(range(episodes), desc="SARSA"):
        ...
        while not done:
            ...
            # Select next action using the same policy (on-policy)
            ...
            if isinstance(state, (int, np.integer)):
                ...
                next_q = Q[next_state_idx][next_action]
                Q[state_idx][action] = current_q + alpha * (reward + gamma * next_q - current_q)
        ...
        if (ep + 1) % print_freq == 0:
            ...
            print(f"Episode {ep+1}: Avg Reward: {avg_reward:.2f}, Avg Steps: {avg_steps:.2f}, Epsilon: {epsilon:.4f}")
