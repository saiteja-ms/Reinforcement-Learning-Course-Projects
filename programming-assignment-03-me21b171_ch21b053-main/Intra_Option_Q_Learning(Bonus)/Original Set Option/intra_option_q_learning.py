...
def intra_option_q_learning(env, num_episodes, alpha, gamma, epsilon):
    # Initialize Q-values table (state x action)
    Q = np.zeros((env.observation_space.n, NUM_ACTIONS))  # (state x action)
    rewards = []

    for episode in tqdm(range(num_episodes), desc="Running Intra-Option Q-Learning", unit="episode"):
...
def plot_rewards(rewards, title="Intra-Option Reward Plot for Original Option Set"):
